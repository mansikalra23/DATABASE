create database company1;  -- to create a database
USE company1; -- to select a databse
...
from employee e
join bank.branch b
on e.ecode=b.ecode; -- joining tables from different databases

select e.ecode, fname, b.branch_name, dname
from employee e
join bank.branch b
	on e.ecode=b.ecode
join department d
	on e.depid=d.depid; -- joining three tables
...
select last_insert_id(); -- returns the first automatically generated integer

create table department_copy as
select * from department; -- generate copy of a table; with no primary key
